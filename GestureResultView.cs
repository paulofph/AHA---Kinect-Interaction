...
namespace Microsoft.Samples.Kinect.DiscreteGestureBasics
{
    ...
    public sealed class GestureResultView : INotifyPropertyChanged
    {
        ...
        private readonly ImageSource seatedImage = new BitmapImage(new Uri(@"Images\Jump1.png", UriKind.Relative));

        /// <summary> Image to show when the 'detected' property is false for a tracked body </summary>
        private readonly ImageSource notSeatedImage = new BitmapImage(new Uri(@"Images\Jump1Not.png", UriKind.Relative));
        ...
        private string trackedGestureName = "Not Tracked";
        ...
        public string TrackedGestureName
        {
            get
            {
                return this.trackedGestureName;
            }

            private set
            {
                if (this.trackedGestureName != value)
                {
                    this.trackedGestureName = value;
                    this.NotifyPropertyChanged();
                }
            }
        }
        ...
        public void UpdateGestureResult(bool isBodyTrackingIdValid, bool isGestureDetected, float detectionConfidence, string gestureName)
        {
            ...
            if (!this.IsTracked)
            {
                ...
                this.TrackedGestureName = "Not Tracked";
            }
            else
            {
                ...
                if (this.Detected)
                {
                    ...
                    this.TrackedGestureName = gestureName;
                }
                else if (gestureName.Equals(this.TrackedGestureName))
                {
                    this.ImageSource = this.notTrackedImage;
                }
            }
